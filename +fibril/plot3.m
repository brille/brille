...
function plot3(bzSTAR,varargin)
    defs=struct('facecolor','none',...
                ...
                'units','invA',...
                'origin',[0,0,0]);
    [~,kwds]=fibril.parse_arguments(varargin,defs,{'showgrid','fullgrid'});
    ph = ishold();
    hold on;

    bzgtype = 'py.fibril._fibril.BZGridQ';
    isgrid = isa(bzSTAR,bzgtype) || isa(bzSTAR, [bzgtype 'complex']);
    bzmtype = 'py.fibril._fibril.BZMeshQ';
    ismesh = isa(bzSTAR, bzmtype) || isa(bzSTAR, [bzmtype 'complex']);
    bztype = 'py.fibril._fibril.BrillouinZone';
    if isa(bzSTAR, bztype)
        bz = bzSTAR;
    elseif isgrid || ismesh;
        bz = bzSTAR.BrillouinZone;
    end
    assert( exist('bz','var')==1, 'The first input must be a fibril.BrillouinZone or fibril.BZ* object');
    ...
    for i=1:size(faces,1)
        perm = fibril.p2m(py.numpy.array(v_p_f(i))) + 1; % +1 converts indexing
