...
% The Fibril object holds one
% py.fibril._fibril.BZGridQ or py.fibril._fibril.BZGridQE object,
...
classdef Fibril < handle
    ...
    methods
        function obj = Fibril(ingrid,varargin)
            kdef = struct('fill',@(x)(1+0*x),...
                          ...
                          'Qscale',eye(4),...
                          'Qtrans',eye(4));
            [args,kwds]=fibril.parse_arguments(varargin,kdef,{'rlu'});
            g3type = 'py.fibril._fibril.BZGridQ';  % or py.fibril._fibril.BZGridQcomplex
            g4type = 'py.fibril._fibril.BZGridQE'; % or py.fibril._fibril.BZGridQEcomplex
            ...
            if ~isempty(kwds.model) && ischar(kwds.model)
                switch lower(kwds.model)
                    case 'spinw'
                        nfill = 2;
                        if obj.nFillers==1
                            obj.filler = [ obj.filler {@fibril.modesort} ];
                            obj.nFillers=2;
                        end
                        rlu = true;
