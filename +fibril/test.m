...
[d,r] = fibril.lattice([2*pi,2*pi,2*pi],[90,90,120],'direct');
...
if ~throws_message('A single py.fibril._fibril.Reciprocal lattice is required as input',@fibril.brillouinzone,d)
    return;
end
bz = fibril.brillouinzone(r);
...
if ~throws_message('A single py.fibril._fibril.BrillouinZone is required as input',@fibril.BZGridQ,r)
    return;
end
bzg = fibril.BZGridQ(bz,'N',[5,5,5]);
...
% more tests?
[~,r]=fibril.lattice(2*pi*[1,1,1],90*[1,1,1]);
bz=fibril.brillouinzone(r);
bzg = fibril.BZGridQ(bz,'N',[10,10,10]);
