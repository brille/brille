...
[args,kwds]=fibril.parse_arguments(varargin,kdef,{'degree','radian','direct','reciprocal'});
...
pylens =fibril.m2p( lens(1:3) );
pyangs =fibril.m2p( angs(1:3) );

if kwds.direct && ~kwds.reciprocal
    dlat = py.fibril.Direct(pylens, pyangs, kwds.spgr);
    rlat = dlat.star();
elseif kwds.reciprocal && ~kwds.direct
    rlat = py.fibril.Reciprocal(pylens, pyangs, kwds.spgr);
    dlat = rlat.star();
end
end
