...
"""
Define a class FibEu to act as the interface between fibril and Euphonic.
...
import fibril as sbz
from fibril.evn import degenerate_check
...
class FibEu:
    ...
    fibril provides classes to hold arbitrarily-shaped data at the points of a
    grid filling the first irreducible Brillouin zone of the primitive lattice
    reciprocal unit cell. The fibril classes can then use linear interpolation
    ...
    to construct the primitive first irreducible Brillouin zone and a fibril
    grid. The gridded-points are then used by the Euphonic object to calculate
    ωᵢ(q) and ϵᵢⱼ(q), which are placed in the fibril object at their respective
    grid points. When an external request is made to the FibEu object to
    calculate Sᵢ(Q) it first uses the filled fibril object to interpolate ωᵢ(Q)
    ...
    def __check_if_cell_is_primitive(self):
        ...
        if sbz.Direct(lattice_vectors).isapprox(sbz.Direct(prim_lv)):
            ...
            # rotation/transformation matrices which fibril doesn't know about
            if self.data_cell_is_primitive is None:
    ...
    def __define_grid_or_mesh(self, mesh=False, trellis=False, nest=False, **kwds):
        ...
        lattice_vectors = (self.data.cell_vec.to('angstrom')).magnitude
        # And we can check whether there's anything to do using fibril
...
def sho(x_0, x_i, y_i, fwhm, t_k):
    ...
    # if the fibril object is holding complex values (it is) then its returned
