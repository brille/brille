...
def plot(*args, **kwds):
    bz_types = (fibril.BrillouinZone,
                fibril.BZGridQcomplex, fibril.BZGridQ,
                fibril.BZMeshQcomplex, fibril.BZMeshQ,
                fibril.BZNestQcomplex, fibril.BZNestQ,
                fibril.BZTrellisQcomplex, fibril.BZTrellisQ)
    if len(args) is 1:
        if isinstance(args[0], bz_types):
            return plot_bz(*args, **kwds)
        if isinstance(args[0], fibril.Polyhedron):
...
def plot_bz(bz, axs=None, origin=None, Q=None, units='invA', irreducible=True,
    ...
    axs = _check_axes(axs)
    types_with_points = (fibril.BZGridQcomplex, fibril.BZGridQ,
                         fibril.BZMeshQcomplex, fibril.BZMeshQ,
                         fibril.BZNestQcomplex, fibril.BZNestQ,
                         fibril.BZTrellisQcomplex, fibril.BZTrellisQ)
