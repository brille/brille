...
class Lattice(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.cmo_translations = np.array([[0.0, 0.0, 0.0] for _ in self.cmo_rotations])

        self.cmo_avg_positions = np.array(
            [
    ...
    def test_a_init2(self):
        ...
        b = np.pi / 2
        v = 1.0
        lat = br_mod.Lattice([a, a, a], [b, b, b])  # compute the volume when required
        self.assertEqual(lat.a, a)
        self.assertEqual(lat.b, a)
        self.assertEqual(lat.c, a)
        self.assertEqual(lat.alpha, b)
        self.assertEqual(lat.beta, b)
        self.assertEqual(lat.gamma, b)
        self.assertAlmostEqual(lat.volume, v)  # since 0.9999999999999999 != 1.0

    def test_a_init3(self):
        a = (1, 1, 1)
        b = np.array([1, 1, 1]) * np.pi / 2
        v = 1.0
        lat = br_mod.Lattice(a, b)
        self.assertEqual(lat.a, a[0])
        self.assertEqual(lat.b, a[1])
        self.assertEqual(lat.c, a[2])
        self.assertEqual(lat.alpha, b[0])
        self.assertEqual(lat.beta, b[1])
        self.assertEqual(lat.gamma, b[2])
        self.assertAlmostEqual(lat.volume, v)  # since 0.9999999999999999 != 1.0
    ...
    def test_b_tensors(self):
        ...
        cmt = np.array(
            [
                [a[0] ** 2, a[0] * a[1] * np.cos(b[2]), 0],
                [a[0] * a[1] * np.cos(b[2]), a[1] ** 2, 0],
                [0, 0, a[2] ** 2],
            ]
        )
        self.assertAlmostEqual((lat.get_covariant_metric_tensor() - cmt).sum(), 0)
        cnt = np.linalg.inv(cmt)
        self.assertAlmostEqual((lat.get_contravariant_metric_tensor() - cnt).sum(), 0)
    ...
    def test_g_wrapped_constructor(self):
        ...
        lat = br_py.Lattice(
            self.cmo_vectors, symmetry=br_mod.Symmetry(*st), basis=br_mod.Basis(*bt)
        )
        self.assertTrue(np.allclose(lat.basis.positions, self.cmo_positions))
        ...
        lat = br_py.Lattice(
            self.cmo_vectors, basis=bt, snap_to_symmetry=False, symmetry=st
        )
        self.assertTrue(np.allclose(lat.basis.positions, self.cmo_avg_positions))
