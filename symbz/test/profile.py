...
for module in [('euphonic', 'data', 'interpolation'), ('symbz', 'euphonic')]:
...
def load_interpolation_data(named):
    """Load a data file from the test submodule's folder"""
...
it = np.nditer([smat, tmat, None, None])
for s, t, i, ie in it:
    se = SymEu(nb, mesh=True, max_size=s, lattice_ratio=t)
    tictoc.tic()
    while not (tictoc.elapsed() > 10 or tictoc.relative_uncertainty() < 0.1):
    # while not (tictoc.elapsed() > 1 or tictoc.relative_uncertainty() < 0.5):
        se(qxyz)
    ...
    print('size {:6.4f} ratio {:6.4f}: <time>={:5.3f}({:5.3f}) sum(time)={:5.2f} sec'.format(s,t,i,ie,tictoc.elapsed()))
interpolation_time = it.operands[2]
interpolation_uncertainty = it.operands[3]

pp.figure()
pcolormesh(smat, tmat, interpolation_time)
pp.xlabel(r'maximum tetrahedral volume / $\AA^{-3}$')
pp.ylabel(r'location trellis spacing / maximum tetrahedral circumsphere radius')

pp.figure()
for i, value in enumerate(trellis_r):
    pp.errorbar(1/max_sizes, interpolation_time[i], yerr=interpolation_uncertainty[i], marker='o', linestyle='-', label='{:8.6f}'.format(value))
pp.xlabel(r'inverse maximum tetrahedral volume / $\AA^3$')
pp.ylabel(r'BZMeshQ object interpolation time / s')
pp.legend(title='[trellis spacing]/[maximum circumsphere radius]')

pp.figure()
for i, value in enumerate(max_sizes):
    pp.errorbar(1/trellis_r**3, interpolation_time[:,i], yerr=interpolation_uncertainty[:,i], marker='o', linestyle='-', label='{:8.6f}'.format(value))
pp.xlabel(r'[maximum tetrahedral circumsphere radius]/[trellis spacing]')
pp.ylabel(r'BZMeshQ object creation time / s')
pp.legend(title=r'maximum tetrahedral volume / $\AA^{-1}$')
