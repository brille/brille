...
Array<T> Array<T>::view() const{
  return Array<T>(_offset, _shape, _stride, _data, false);
}
template<class T>
Array<T> Array<T>::view(const ind_t i) const {
  if (i<_shape[0]){
    ...
    return Array<T>(oofst, osize, _data, false);
  }
...
template<class T>
Array<T> Array<T>::extract(const ind_t i) const{
...
template<class T>
Array<T> Array<T>::extract(const Array<bool>& i) const {
...
template<class T>
Array<T> Array<T>::extract(const std::vector<bool>& i) const {
  if (i.size() >= _shape[0])
    throw std::runtime_error("Boolean Array extraction requires no more bools than the first Array dimension");
  shape_t osize{_shape};
...
bool Array<T>::set(const ind_t i, const Array<T>& in){
  ...
  for (ind_t dim=1; dim<ndim; ++dim) if (_shape[i] != inshape[i])
...
std::string Array<T>::to_string() const{
  ...
  for (auto sub: SubIt<ind_t>(_shape)){
    ...
    out += std::to_string(_data[this->s2s_d(sub)]);
...
Allocate a new SharedSpan<T> and copy the overlapping region. The new shape
...
Array<T>::resize(const shape_t& ns, const T init){
  _shape = ns; // this seems like a bad thing to do without lots of checks
  return _data.resize(ns, init);
}
template<class T> template<class I>
ind_t
Array<T>::resize(const I ns, const T init){
  _shape[0] = static_cast<ind_t>(ns); // this seems like a bad thing to do without lots of checks
  return _data.resize(_shape, init);
}

template<class T>
Array<T>& Array<T>::append(const ind_t dim, const Array<T>& extra){
  assert(this != &extra);
  ...
  // and resizing this Array to the new size copies existing entries
  this->resize(eshape);
...
bool Array<T>::all(const ind_t n) const{
  ...
  ind_t count = n < nml ? n : nml;
  for (ind_t i=0; i<count; ++i) if (!_data[this->l2s_d(i)]) return false;
  return true;
}
template<class T>
bool Array<T>::any(const ind_t n) const{
  ind_t nml = this->numel();
  ind_t count = n < nml ? n : nml;
  for (ind_t i=0; i<count; ++i) if (_data[this->l2s_d(i)]) return true;
  return false;
}
template<class T>
bool Array<T>::all(const T val, const ind_t n) const{
  ind_t nml = this->numel();
  ind_t count = n < nml ? n : nml;
  for (ind_t i=0; i<count; ++i) if (val != _data[this->l2s_d(i)]) return false;
  return true;
}
template<class T>
bool Array<T>::any(const T val, const ind_t n) const{
  ind_t nml = this->numel();
  ind_t count = n < nml ? n : nml;
  for (ind_t i=0; i<count; ++i) if (val == _data[this->l2s_d(i)]) return true;
  return false;
}
...
Array<T>::count(const ind_t n) const{
  ...
  ind_t no = n < el ? n : el;
  ind_t count{0};
  for (ind_t i=0; i<no; ++i) if (_data[this->l2s_d(i)]) ++count;
  return count;
}
template<class T>
ind_t
Array<T>::count(const T val, const ind_t n) const{
  ...
  ind_t no = n < el ? n : el;
  ind_t count{0};
  for (ind_t i=0; i<no; ++i) if (val == _data[this->l2s_d(i)]) ++count;
  return count;
}
...
Array<T>::first(const ind_t n) const{
  ind_t el = this->numel();
  ind_t no = n < el ? n : el;
  for (ind_t i=0; i<no; ++i) if (_data[this->l2s_d(i)]) return i;
  return no;
}
template<class T>
ind_t
Array<T>::first(const T val, const ind_t n) const{
  ind_t el = this->numel();
  ind_t no = n < el ? n : el;
  for (ind_t i=0; i<no; ++i) if (val == _data[this->l2s_d(i)]) return i;
  return no;
}
template<class T>
ind_t
Array<T>::last(const ind_t n) const{
  ind_t el = this->numel();
  ind_t no = n < el ? n : el;
  for (ind_t i=no; i--;) if (_data[this->l2s_d(i)]) return i;
  return no;
}
template<class T>
ind_t
Array<T>::last(const T val, const ind_t n) const{
  ind_t el = this->numel();
  ind_t no = n < el ? n : el;
  for (ind_t i=no; i--;) if (val == _data[this->l2s_d(i)]) return i;
  return no;
}

template<class T>
Array<int> Array<T>::round() const{
  SharedSpan<int> outdata(_shape, _stride);
  for (auto x: SubIt(outdata.shape()))
    outdata[x] = static_cast<int>(std::round(_data[this->s2s_d(x)]));
  return Array<int>(_shape, _stride, outdata);
}
template<class T>
Array<int> Array<T>::floor() const{
  SharedSpan<int> outdata(_shape, _stride);
  for (auto x: SubIt(outdata.shape()))
    outdata[x] = static_cast<int>(std::floor(_data[this->s2s_d(x)]));
  return Array<int>(_shape, _stride, outdata);
}
template<class T>
Array<int> Array<T>::ceil() const{
  SharedSpan<int> outdata(_shape, _stride);
  for (auto x: SubIt(outdata.shape()))
    outdata[x] = static_cast<int>(std::ceil(_data[this->s2s_d(x)]));
  return Array<int>(_shape, _stride, outdata);
}

template<class T>
Array<T> Array<T>::sum(const ind_t dim) const{
  size_t ndim = _shape.size();
  assert(dim < ndim);
  ...
  Array<T> out(shape_t(ndim, 0), osize, ostride);
  for (auto oidx: SubIt<ind_t>(osize)) {
    T tmp{0};
    auto idx = oidx;
    for (ind_t i=0; i<_shape[dim]; ++i) {
      idx[dim] = i;
      tmp += _data[this->s2s_d(idx)];
    }
    out[oidx] = tmp;
  }
  return out;
...
template<class T>
Array<T> Array<T>::prod(const ind_t dim) const{
  size_t ndim = _shape.size();
  assert(dim < ndim);
  shape_t ostride = this->stride();
  shape_t osize = this->shape();
...
template<class T>
Array<T> Array<T>::min(const ind_t dim) const{
  size_t ndim = _shape.size();
  assert(dim < ndim);
  shape_t ostride = this->stride();
  shape_t osize = this->shape();
...
template<class T>
Array<T> Array<T>::max(const ind_t dim) const{
  size_t ndim = _shape.size();
  assert(dim < ndim);
  shape_t ostride = this->stride();
  shape_t osize = this->shape();
  ...
  shape_t ooffset = shape_t(ndim,0);
  Array<T> out(ooffset, osize, ostride);
  for (auto oidx: SubIt<ind_t>(osize)) {
...
T Array<T>::sum() const{
  T out{0};
  for (auto x : _shape) out += _data[this->s2s_d(x)];
  return out;
}
...
bool Array<T>::all(const brille::cmp expr, const T val) const {
  ...
  for (ind_t k=0; k<no; ++k) if(!op(_data[this->l2s_d(k)], val)) return false;
  return true;
}
template<class T>
...
Array<T>::first(const brille::cmp expr, const T val) const {
  ind_t no = this->numel();
  brille::Comparer<T,T> op(expr);
  for (ind_t k=0; k<no; ++k) if(op(_data[this->l2s_d(k)], val)) return k;
  return no;
}
template<class T>
ind_t
Array<T>::last(const brille::cmp expr, const T val) const {
  ind_t no = this->numel();
  brille::Comparer<T,T> op(expr);
  for (ind_t k=no; k--;) if(op(_data[this->l2s_d(k)], val)) return k;
  return no;
}
template<class T>
ind_t
Array<T>::count(const brille::cmp expr, const T val) const {
  ...
  brille::Comparer<T,T> op(expr);
  ind_t cnt{0};
  for (ind_t k=0; k<no; ++k) if(op(_data[this->l2s_d(k)], val)) ++cnt;
  return cnt;
}

template<class T>
Array<bool> Array<T>::is(const brille::cmp expr, const T val) const {
  Array<bool> out(_shape, _stride, true);
  ind_t no = this->numel();
  brille::Comparer<T,T> op(expr);
  for (ind_t k=0; k<no; ++k) out[k] = op(_data[this->l2s_d(k)], val);
  return out;
}
...
Array<T>::find(const brille::cmp expr, const T val) const {
  ...
  ind_t no = this->numel(), cnt = this_is.count();
  Array<ind_t> out(cnt);
  cnt=0;
  for (ind_t k=0; k<no; ++k) if (this_is[k]) out[cnt++] = k;
  return out;
}

template<class T> template<class R>
Array<bool> Array<T>::is(const brille::cmp expr, const Array<R>& that) const {
  ...
  if (std::equal(_shape.begin(), _shape.end(), tsize.begin())){
    ...
    for (auto sub: SubIt<ind_t>(_shape))
      out[sub] = op(_data[this->s2s_d(sub)], that[sub]);
  } else {
    // Broadcast
...
template<class T> template<class R>
std::vector<bool> Array<T>::is(const brille::cmp expr, const std::vector<R>& val) const{
  assert(2==this->ndim());
  ...
  brille::Comparer<T,R> op(expr);
  for (ind_t i=0; i<_shape[0]; ++i)
    out.push_back(op(_shape[1], this->view(i).data(), _stride[1], val.data(), 1u));
  return out;
}
...
template<class T>
std::vector<bool> Array<T>::is_unique() const{
  assert(2==this->ndim());
  ...
  for (ind_t i=1; i<_shape[0]; ++i){
    bool isu{true};
    for (ind_t j=0; j<i; ++j){
      isu &= op(sz, this->view(i).data(), st, this->view(j).data(), st);
      if (!isu) break;
    }
    out.push_back(isu);
  }
...
Array<T>::unique_idx() const{
  ...
  for (ind_t i=1; i<_shape[0]; ++i){
    ind_t idx{i};
    for (ind_t j=0; j<i; ++j)
    if(j==out[j] && op(sz, this->view(i).data(), st, this->view(j).data(), st)){
      idx=j;
      break;
    }
    out.push_back(idx);
...
T Array<T>::dot(ind_t i, ind_t j) const {
  ...
  prod(sz, prods.data(), 1u, this->view(i).data(), st, this->view(j).data(), st);
...
Array<T> Array<T>::operator-() const{
  return Array<T>(_offset, _shape, _stride, -_data) ;
}
...
bool Array<T>::swap(const ind_t a, const ind_t b){
  ...
  for (auto aidx: itr){
    ...
    shape_t s2s_a = this->s2s_d(aidx);
    shape_t s2s_b = this->s2s_d(bidx);
    // perform the actual swap:
    T store = _data[s2s_a];
    _data[s2s_a] = _data[s2s_b];
    _data[s2s_b] = store;
  }
  return true;
}
...
bool Array<T>::swap(ind_t i, ind_t a, ind_t b){
  ...
  T store = _data[this->s2s_d(ia)];
  _data[this->s2s_d(ia)] = _data[this->s2s_d(ib)];
  _data[this->s2s_d(ib)] = store;
  return true;
}
...
std::vector<T> Array<T>::to_std() const {
  std::vector<T> out;
  for (auto x: ArrayIt<T>(*this)) out.push_back(x);
  return out;
}
...
Array<T>& Array<T>::operator X (const T& val){\
  ...
  for (ind_t i=0; i<nel; ++i) _data[this->l2s_d(i)] X val;\
  return *this;\
}
SCALAR_INPLACE_OP(+=)
SCALAR_INPLACE_OP(-=)
...
Array<T>& Array<T>::operator X (const Array<R>& val){\
  ...
  for (auto [ox, tx, vx]: itr) _data[this->s2s_d(tx)] X val[vx];\
  return *this;\
}
ARRAY_INPLACE_OP(+=)
ARRAY_INPLACE_OP(-=)
