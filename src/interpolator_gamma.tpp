...
bool Interpolator<T>::rip_gamma_complex(
  ...
  auto make_task = [&](const size_t thread) {
    ...
    return [&, first=f, last=l, b_=branches(), s_=branch_span()] () {
      for (size_t i=first; i<last; ++i) {
        std::vector<T> tA; // temporary array storage
        auto xi = x.ptr(i);
        auto Rii = ridx[i];
        auto iRii = invRidx[i];
        for (ind_t b=0; b<b_; ++b){
          ...
          if (no[1]>0){
            ...
            for (ind_t k=0; k<no[1]; ++k){
              ...
              brille::utils::mul_mat_vec(t0, 3u, rot[iRii].data(), xi+o);
              const auto v0idx = 3u*pgt.F0(k, iRii); // ×3u to account for stride
              const T phase = e_iqd_gt(i, k, iRii);
              for (int j=0; j<3; ++j) tA[v0idx+j] = phase*t0[j];
              o += 3u; // increase the offset for each vector
            }
            for (ind_t j=0; j<no[1]*3u; ++j) xi[o0+j] = tA[j];
          ...
          if (no[2]>0){
            if (tA.size() < no[2]*9u) tA.resize(no[2]*9u);
            for (ind_t n=0; n<Nmat; ++n){
              const T Rph = e_iqd_gt(i, n, Rii);
              const ind_t v = static_cast<ind_t>(pgt.F0(n, Rii));
              for (ind_t m=0; m<Nmat; ++m){
                const T iRph = e_iqd_gt(i, m, iRii);
                const ind_t k = static_cast<ind_t>(pgt.F0(m, iRii));
                // Calculate R⁻¹*M*R in two steps
                // first calculate M*R, storing in t0
                utils::mul_mat_mat(t0, 3u, xi+o+9u*(n*Nmat+m), rot[Rii].data());
                // next calculate R⁻¹*t0, storing in the temporary all matrix array
                utils::mul_mat_mat(t1, 3u, rot[iRii].data(), t0);
                // include the R * R⁻¹ phase factor
                for (int j=0; j<9; ++j) tA[(v*Nmat+k)*9u+j] = Rph*iRph*t1[j];
              }
            }
  ...
  for (size_t thread=0; thread<workers; ++thread) pool->enqueue(make_task(thread));
  profile_update("    Wait Interpolator::rip_gamma_complex treads");
  pool->wait();
  profile_update("  End Interpolator::rip_gamma_complex method");
  return true;
}
