...
brille::Array<int> make_relative_neighbour_indices(const int extent){
  ...
  for (auto i: vec) for (auto j: vec) for (auto k: vec) if (i||j||k)
    o[n++] = {{i,j,k}};
  return brille::Array<int>::from_std(o);
}

brille::Array<int> make_relative_neighbour_indices_prime(const int extent){
  auto vec = all_from_extent(extent);
  size_t num = vec.size();
  size_t n{0};
...
brille::Array<int> make_relative_neighbour_indices4(const int extent){
