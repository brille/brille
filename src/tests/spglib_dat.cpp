...
// a nicer way to access the static const symmetry operations and indexing information
struct SPG_Magic_Numbers{
  static const int operation[7389];
  static const int count_first[531][2];
};
...
int get_numbered_operation(std::array<int,9> & rot, std::array<double,3> & trans, const int idx)
{
  ...
  int r = SPG_Magic_Numbers::operation[idx] % p3[0]; // % 3⁹
  for (int i=0; i<9; ++i) {
    rot[i] = (r % p3[i]) / p3[i + 1] - 1;
  }
  // powers of 12: 12³, 12², 12¹, 12⁰
  int p12[4]{1728, 144, 12, 1};
  int t = SPG_Magic_Numbers::operation[idx] / p3[0];
  for (int i=0; i<3; ++i) {
...
brille::Symmetry make_spacegroup_symmetry_object(const int hall_number)
{
  using brille::Symmetry;
  if (hall_number<1 || hall_number>530) return Symmetry();
  const int count{SPG_Magic_Numbers::count_first[hall_number][0]};
  const int first{SPG_Magic_Numbers::count_first[hall_number][1]};
  Symmetry symmetry;
...
const int SPG_Magic_Numbers::operation[7389] = {
  0       ,  /* dummy */
...
// Each count_first[hall_number] is [number_of_operations, index_of_first_operation]
const int SPG_Magic_Numbers::count_first[531][2] = {
  {   0,    0}, /*   0 */
  {   1,    1}, /*   1 */
  {   2,    2}, /*   2 */
  {   2,    4}, /*   3 */
