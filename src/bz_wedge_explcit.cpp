...
void BrillouinZone::wedge_explicit(void){
...
double c_beta = std::cos(beta), s_alpha = std::sin(beta);
double c_gamma = std::cos(gamma), s_alpha = std::sin(gamma);
...
double zeta, eta, mu, nu, dlt, psi, phi, omega, rho;
  ...
  switch (holo){
    ...
    case Holohedry::monoclinic:
      switch (brav){
        case Bravais::I: // same as Primitive for monoclinic?
        case Bravais::P:
