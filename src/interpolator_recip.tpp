...
bool Interpolator<T,P>::rip_recip(
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t v=0; v<no[1]; ++v){
          T *xptr = xib.ptr(o);
          brille::utils::mul_mat_vec(tmp_v, 3u, transpose(ptsym.get(r[i])).data(), xptr);
        ...
        for (ind_t m=0; m<no[2]; ++m){
          ...
          brille::utils::mul_mat_mat(tmp_m, 3u, xptr, transpose(ptsym.get(invR[i])).data());
          // next calculate R*tmp_m, storing back in the x array
          brille::utils::mul_mat_mat(xptr, 3u, transpose(ptsym.get(r[i])).data(), tmp_m);
...
template<class T, class P> template<class Q>
bool
Interpolator<T,P>::rip_recip_vec(data_t<T,Q>& x, const PointSymmetry& ptsym, const std::vector<size_t>& r, const std::vector<size_t>& invR, const int nthreads) const {
  profile_update("Start Interpolator::rip_recip_vec method");
  omp_set_num_threads( (nthreads>0) ? nthreads : omp_get_max_threads() );
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t v=0; v<v_; ++v){
          ...
          brille::utils::mul_mat_vec(tmp_v, 3u, transpose(ptsym.get(r[i])).data(), xibv.ptr());
          for (ind_t j=0; j<3u; ++j) xibv.val(j) = tmp_v[j];
        }
      }
    }
  }
  profile_update("  End Interpolator::rip_recip_vec method");
  return true;
}

template<class T, class P> template<class Q>
bool
Interpolator<T,P>::rip_recip_mat(data_t<T,Q>& x, const PointSymmetry& ptsym, const std::vector<size_t>& r, const std::vector<size_t>& invR, const int nthreads) const {
  profile_update("Start Interpolator::rip_recip_mat method");
  omp_set_num_threads( (nthreads>0) ? nthreads : omp_get_max_threads() );
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t m=0; m<m_; ++m){
          ...
          // Calculate R*M*R⁻¹ in two steps
          // first calculate M*R⁻¹, storing in tmp_m
          brille::utils::mul_mat_mat(tmp_m, 3u, xibm.ptr(), transpose(ptsym.get(invR[i])).data());
          // next calculate R*tmp_m, storing back in the x array
          brille::utils::mul_mat_mat(xibm.ptr(), 3u, transpose(ptsym.get(r[i])).data(), tmp_m);
        }
      }
    }
  }
  profile_update("  End Interpolator::rip_recip_mat method");
  return true;
}
