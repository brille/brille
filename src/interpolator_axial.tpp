...
#else
#pragma omp parallel for default(none) shared(b_,x,ptsym,r,invR,detR) private(tmp_v,tmp_m) firstprivate(ident,no,xsize) schedule(static)
#endif
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t v=0; v<no[1]; ++v){
          T *xptr = xib.ptr(o);
          brille::utils::mul_mat_vec(tmp_v, 3u, ptsym.get(invR[i]).data(), xptr);
          for (int j=0; j<3; ++j) xptr[j] = detR[i]*tmp_v[j];
        ...
        for (ind_t m=0; m<no[2]; ++m){
          ...
          brille::utils::mul_mat_mat(tmp_m, 3u, xptr, ptsym.get(r[i]).data());
          // next calculate R⁻¹*tmp_m, storing back in the x array
          brille::utils::mul_mat_mat(xptr, 3u, ptsym.get(invR[i]).data(), tmp_m);
...
#if defined(__GNUC__) && !defined(__llvm__) && __GNUC__ < 9
// otherwise gcc complains that const b_ is *already* shared
#pragma omp parallel for default(none) shared(x,ptsym,r,invR,detR) private(tmp_v) firstprivate(ident,xsize) schedule(static)
#else
#pragma omp parallel for default(none) shared(b_,v_,x,ptsym,r,invR,detR) private(tmp_v) firstprivate(ident,xsize) schedule(static)
#endif
  for (long long si=0; si<xsize; ++si){
    ind_t i = brille::utils::s2u<ind_t, long long>(si);
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t v=0; v<v_; ++v){
          ...
          brille::utils::mul_mat_vec(tmp_v, 3u, ptsym.get(invR[i]).data(), xibv.ptr());
          for (ind_t j=0; j<3u; ++j) xibv.val(j) = detR[i]*tmp_v[j];
        }
      }
    }
  }
  profile_update("  End Interpolator::rip_axial_vec method");
  return true;
}
    ...
    if (!brille::approx::matrix(3, ident.data(), ptsym.get(r[i]).data())){
      for (ind_t b=0; b<b_; ++b){
        ...
        for (ind_t m=0; m<m_; ++m){
          ...
          // Calculate R*M*R⁻¹ in two steps
          // first calculate M*R⁻¹, storing in tmp_m
          brille::utils::mul_mat_mat(tmp_m, 3u, xibm.ptr(), ptsym.get(r[i]).data());
          // next calculate R*tmp_m, storing back in the x array
          brille::utils::mul_mat_mat(xibm.ptr(), 3u, ptsym.get(invR[i]).data(), tmp_m);
        }
      }
    }
  }
  profile_update("  End Interpolator::rip_axial_mat method");
  return true;
}
