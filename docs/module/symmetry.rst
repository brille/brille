...
The matrix must be the identity matrix, a rotation matrix, or a rotoinversion matrix expressed as integer multiples of the realspace basis vectors.
...
with the appropriate symmetry operations without also specifiying, e.g., the spacegroup::
...
which has the avantage of representing all unique crystallographic lattices as an encoded set of generators.
